...
describe('HybridSearch', () => {
  ...
  describe('subject filtering', () => {
    beforeEach(() => {
      ...
      store.add({
        id: 'hamza-work',
        content: 'Occupation: Globex',
        type: 'fact',
        timestamp: new Date(),
        sessionId: 's1',
        metadata: { subject: 'Bob' },
      });
    });

    it('should filter by subject when specified', () => {
      const results = search.search('Acme Corp', { subject: 'user' });
    ...
    it('should filter by third party subject', () => {
      const results = search.search('Globex', { subject: 'Bob' });
    ...
    it('should be case insensitive for subject matching', () => {
      const results = search.search('Globex Occupation', { subject: 'hamza' }); // lowercase

      expect(results.length).toBe(1);
      expect(results[0].entry.metadata?.subject).toBe('Bob');
    });
  });
});
