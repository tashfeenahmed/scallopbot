...
describe('Inference Engine', () => {
  describe('inferConnections', () => {
    it('should infer flatmate lives in same location as user', async () => {
      const provider = createMockProvider(JSON.stringify({
        inferences: [
          {
            content: 'Bob lives in Springfield',
            subject: 'Bob',
            category: 'location',
            derivedFrom: ['fact-1', 'fact-2'],
            confidence: 0.9,
            reason: 'Flatmates typically live together'
          }
        ]
      }));
      const classifier = createRelationshipClassifier(provider);

      const inferences = await classifier.inferConnections([
        { id: 'fact-1', content: 'Lives in Springfield', subject: 'user', category: 'location' },
        { id: 'fact-2', content: 'Flatmate is Bob', subject: 'user', category: 'relationship' },
      ]);

      expect(inferences.length).toBeGreaterThan(0);
      expect(inferences[0].content).toContain('Bob');
      expect(inferences[0].content).toContain('Springfield');
    });

    it('should not infer connections without sufficient facts', async () => {
      const provider = createMockProvider('{"inferences": []}');
      const classifier = createRelationshipClassifier(provider);
