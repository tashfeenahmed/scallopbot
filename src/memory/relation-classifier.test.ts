  ...
  describe('inferConnections', () => {
    it('should infer flatmate lives in same location as user', async () => {
      const provider = createMockProvider(JSON.stringify({
        inferences: [
          {
            content: 'Hamza lives in Wicklow',
            subject: 'Hamza',
            category: 'location',
      ...
      const inferences = await classifier.inferConnections([
        { id: 'fact-1', content: 'Lives in Wicklow', subject: 'user', category: 'location' },
        { id: 'fact-2', content: 'Flatmate is Hamza', subject: 'user', category: 'relationship' },
      ]);

      expect(inferences.length).toBeGreaterThan(0);
      expect(inferences[0].content).toContain('Hamza');
      expect(inferences[0].content).toContain('Wicklow');
    });
