...
describe('RelationGraph LLM-based classification', () => {
  describe('no LLM provider (backward compatible regex)', () => {
    it('should use regex classification when no classifierProvider is given', async () => {
      ...
      const embedder = createMockEmbedder(new Map([
        ['Lives in Metropolis', emb.a],
        ['Lives in Springfield', emb.b],
      ]));
      ...
      const newMem = makeMemory({ id: 'new1', content: 'Lives in Metropolis', embedding: emb.a });
      const existing = makeMemory({ id: 'old1', content: 'Lives in Springfield', embedding: emb.b });
  ...
  describe('LLM provider + multiple candidates', () => {
    it('should use LLM classifyBatch() for multiple candidates', async () => {
      ...
      const newMem = makeMemory({ id: 'new1', content: 'Lives in Metropolis', embedding: emb.a });
      const candidates = [
        makeMemory({ id: 'old1', content: 'Lives in Springfield', embedding: emb.b }),
  ...
  describe('LLM classification mapping', () => {
    ...
    it('should map EXTENDS to DetectedRelation with relationType EXTENDS', async () => {
      ...
      const newMem = makeMemory({ id: 'new1', content: 'Bob works at Google', embedding: emb.a });
      const existing = makeMemory({ id: 'old1', content: 'Flatmate is Bob', embedding: emb.b });
