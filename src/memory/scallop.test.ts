...
describe('ScallopMemoryStore', () => {
  ...
  it('should search memories', async () => {
    await store.add({ userId: 'user1', content: 'User works at Acme Corp', category: 'fact' });
    ...
    const results = await store.search('Acme Corp', { userId: 'user1' });
...
describe('Scenario: Memory fusion in deep tick', () => {
  ...
  it('should handle LLM fusion failure gracefully', async () => {
    ...
    try {
      ...
      const mem1 = addOldMemory(db, 'User works at a startup company in Metropolis', { accessCount: 1 });
