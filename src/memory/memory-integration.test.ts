...
describe('Scenario 1: Fact storage, embedding cache & search weights', () => {
  ...
  it('should store facts from a multi-turn conversation and find them via hybrid search', async () => {
    ...
    const mem1 = await store.add({
      ...
      category: 'fact',
      importance: 7,
      sourceChunk: 'I work at Acme Corp as a senior engineer',
    ...
    const workResults = await store.search('Acme Corp engineer', { userId: 'default' });
    expect(workResults.length).toBeGreaterThanOrEqual(1);
    expect(workResults[0].memory.content).toContain('Acme Corp');

    // Search for location
    const locationResults = await store.search('Metropolis', { userId: 'default' });
    expect(locationResults.length).toBeGreaterThanOrEqual(1);
    expect(locationResults[0].memory.content).toContain('Metropolis');
    ...
    expect(mem1.sourceChunk).toBe('I work at Acme Corp as a senior engineer');
    expect(mem2.sourceChunk).toBe('I live in Metropolis');
...
describe('Scenario 3: Corrections and contradiction tracking', () => {
  ...
  it('should track contradictions when a fact is corrected', async () => {
    // Turn 1: User says they live in Metropolis
    ...
    const newFact = await store.add({
      userId: 'default',
      content: 'Lives in Cork (moved from Metropolis)',
      category: 'fact',
      importance: 7,
  ...
  it('should track correction chains across multiple updates', async () => {
    ...
    // Correction 1: Actually works at Acme Corp
    ...
    // Correction 2: Left Acme Corp, now freelancing
    const v3 = await store.add({
      userId: 'default',
      content: 'Now freelancing after leaving Acme Corp',
      category: 'fact',
      learnedFrom: 'correction',
    });
...
describe('Scenario 4: Session summaries and past-conversation search', () => {
  ...
  it('should generate and store session summaries', async () => {
    ...
    db.createSession(sessionId);
    db.addSessionMessage(sessionId, 'user', 'I work at Acme Corp as a senior engineer');
    ...
    expect(summary!.summary).toContain('Acme Corp');
...
describe('Scenario 5: Tiered consolidation, behavioral patterns & stats', () => {
  ...
  it('should return comprehensive stats via getStats()', async () => {
    ...
    await store.add({ userId: 'default', content: 'Works at Acme Corp', category: 'fact' });
    await store.add({ userId: 'default', content: 'Prefers coffee', category: 'preference' });
    await store.add({ userId: 'default', content: 'Lives in Metropolis', category: 'fact' });
    ...
    await store.search('Acme Corp', { userId: 'default' });
    await store.search('Acme Corp', { userId: 'default' }); // should hit cache
  ...
  it('should handle a natural 5-turn conversation end-to-end', async () => {
    ...
    const provider = createSmartProvider([
      ...
      { facts: [
        ...
        { content: 'Is a content creator', subject: 'Jamie', category: 'work' },
      ]},
      // Turn 4: Travel plans
      { facts: [
    ...
    const r1 = await simulateTurn(
      ...
      "Hi! I'm a senior engineer at Acme Corp, and I live in Metropolis.",
      "Nice to meet you! Metropolis is a great city."
    );
    ...
    const r3 = await simulateTurn(
      extractor, db, sessionId, 'default',
      "My wife Jamie is a content creator. She makes great videos!",
    ...
    const workSearch = await store.search('Acme Corp', { userId: 'default' });
...
describe('Optimizations: reduced embedding & search calls', () => {
  ...
  it('should skip embed() when embedding is passed to add()', async () => {
    const precomputed = await rawEmbedder.embed('Works at Acme Corp');
  ...
  it('should skip embed(query) when queryEmbedding is passed to search()', async () => {
    ...
    await store.add({
      userId: 'default',
      content: 'Works at Acme Corp as an engineer',
      category: 'fact',
    });

    // Pre-compute query embedding
    const queryEmb = await rawEmbedder.embed('Acme Corp engineer');
    const callsBefore = (rawEmbedder.embed as ReturnType<typeof vi.fn>).mock.calls.length;

    // Search with pre-computed embedding
    const results = await store.search('Acme Corp engineer', {
      userId: 'default',
