...
describe('findFusionClusters', () => {
  it('returns 1 cluster of 3 when 3 related facts are in dormant range', () => {
    const memories = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', prominence: 0.3, category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works in Metropolis city centre', prominence: 0.25, category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Office is near Central University', prominence: 0.2, category: 'fact' }),
    ];
  ...
  it('returns empty array when 2 related facts are below minClusterSize=3', () => {
    const memories = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', prominence: 0.3, category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works in Metropolis', prominence: 0.25, category: 'fact' }),
    ];
  ...
  it('splits mixed categories in connected component into per-category sub-clusters', () => {
    const memories = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', prominence: 0.3, category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works in Metropolis', prominence: 0.25, category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Metropolis office is great', prominence: 0.2, category: 'fact' }),
  ...
  it('excludes active memories (prominence >= 0.5) even if related', () => {
    const memories = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', prominence: 0.3, category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works in Metropolis', prominence: 0.25, category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Metropolis office hours', prominence: 0.2, category: 'fact' }),
      makeMemory({ id: 'm4', content: 'Just mentioned Metropolis today', prominence: 0.7, category: 'fact' }), // active
    ];
  ...
  it('excludes derived memories even if in dormant range', () => {
    const memories = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', prominence: 0.3, category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works in Metropolis', prominence: 0.25, category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Metropolis-related summary', prominence: 0.2, category: 'fact', memoryType: 'derived' }),
      makeMemory({ id: 'm4', content: 'Metropolis office', prominence: 0.2, category: 'fact' }),
    ];
...
describe('fuseMemoryCluster', () => {
  it('returns fused summary mentioning all facts for 3 facts about same topic', async () => {
    const cluster = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', importance: 7, confidence: 0.9, category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works at a tech company in Metropolis', importance: 5, confidence: 0.8, category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Office is near Trinity College', importance: 6, confidence: 0.85, category: 'fact' }),
    ];

    const llmResponse = JSON.stringify({
      summary: 'Lives and works at a tech company in Metropolis, with office near Trinity College',
      importance: 7,
      category: 'fact',
    });
    ...
    expect(result!.summary).toContain('Metropolis');
  ...
  it('returns null when LLM returns invalid JSON', async () => {
    const cluster = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Works in Metropolis', category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Office in Metropolis', category: 'fact' }),
    ];
  ...
  it('returns null when summary is longer than combined sources (validation)', async () => {
    const cluster = [
      makeMemory({ id: 'm1', content: 'Metropolis', category: 'fact' }),
      makeMemory({ id: 'm2', content: 'Office', category: 'fact' }),
      makeMemory({ id: 'm3', content: 'Work', category: 'fact' }),
    ];

    // Summary is much longer than all sources combined ("Metropolis" + "Office" + "Work" = 16 chars)
...
describe('buildFusionPrompt', () => {
  it('includes all memory contents with categories and importance', () => {
    const cluster = [
      makeMemory({ id: 'm1', content: 'Lives in Metropolis', importance: 7, category: 'fact' }),
    ...
    expect(prompt.messages[0].content).toContain('Lives in Metropolis');
