...
| 4 | Output and tool safety | A single shared outcome brain is the final authority for foreground replies, proactive/scheduled messages, sub-agent completions, workflow actions, progress, and files. It combines bounded live state, strips private reasoning, suppresses stale/duplicate outreach, and records hash-only receipts. Allow/deny policy is enforced again at dispatch, including workflows and subagents. Skill environments use an allowlist and logs/traces redact secrets. The dashboard withholds progress by default, exposes only redacted lifecycle summaries when explicitly made verbose, and removes reasoning/tool/worker protocol from user-facing history. | `src/brain/outcome-brain.test.ts`, `src/gateway/gateway.test.ts`, `src/agent/tool-policy-dispatch.test.ts`, `src/agent/agent.test.ts`, `src/channels/api.test.ts`, `src/skills/executor.test.ts`, `src/security/redaction.test.ts` |
...
| 7 | MCP | Bidirectional. **Client**: the bundled MCP skill supports progressive stdio discovery and calls with owner-only config, explicit per-server tool allowlists, advertised-schema validation, scoped-secret redaction, bounded I/O, and process-tree cleanup. **Server**: `src/mcp-server/` exposes memory to any MCP client over stdio (`memory_store`, `memory_recall`, `memory_temporal`) on the same protocol revision as the client, safe to run alongside the live bot via WAL plus `busy_timeout` and no write transaction held across an await. | `src/skills/bundled/mcp/mcp.test.ts`, `src/mcp-server/mcp-server.test.ts` |
